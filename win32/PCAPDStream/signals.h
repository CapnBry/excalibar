...
public:
    ...
    bool WaitAll(unsigned int timeout=1000)
    {
        ...
        switch(WaitForMultipleObjects(DWORD(signal_list.size()),events,TRUE,timeout))
            {
            case WAIT_TIMEOUT:
            case WAIT_FAILED:
                delete events;
    ...
    wait_any_result_type WaitAny(unsigned int timeout=1000)
    {
        ...
        switch(result=WaitForMultipleObjects(DWORD(signal_list.size()),events,FALSE,timeout))
            {
            case WAIT_TIMEOUT:
            case WAIT_FAILED:
                delete events;
