...
Supported key names (surround these with brackets):
...
begin
  ...
  repeat
    ...
    if AnsiSameText(KeyString, SendKeyRecs[Middle].Name) then begin
       Found := True;
       Result := SendKeyRecs[Middle].VKey;
    end
    else begin
      if AnsiCompareText(KeyString, SendKeyRecs[Middle].Name) > 0 then
        Bottom := Middle
      else
        Top := Middle;
  ...
  if (Result = INVALIDKEY) then
    raise Exception.Create('Invalid keyname [' + KeyString + ']');
end;
  ...
  While (I<L) do begin
    ...
    '[' : begin
            ...
            If (SendKeysString[Succ(I)]='[') then begin
              MKey:=VK_LEFTBRACKET;
                { BRY:  Removed this because we used to use a shift bracket }
    ...
    '~' : begin
            ...
            Inc(I);
             ...
             If (MKey<>INVALIDKEY) then begin
              ...
              raise Exception.Create('Invalid Key [' + SendKeysString[I] + ']');
             Inc(I);
          end;
    end;
  end;
